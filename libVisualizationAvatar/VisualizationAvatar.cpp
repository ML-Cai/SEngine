...
{ \
    ...
    if (intensity > 0.8) \
        ...
        color = vec4(0.1, 0.1, 0.1, 1.0); \
    gl_FragColor = texture2D(uTexture_1, TexCoord ); \
}";
// ---------------------------------------------------------------------------------
...
{
     GLfloat LightPosition[] = {1000.0f, 2000.0f, -2000.0f};
    int i;
...
#ifdef ASD
    int ci;
    float nVertex_0[3], nVertex_1[3];
    struct SkeletonChainContext *SkeletonChain_ptr = fBone;
    struct SkeletonVertexChainContext *VertexChain_ptr = SkeletonChain_ptr->VertexChain;

    /* Update Skeleton Bone Vertex */
    if(fBone != &MasterSkeletonCtx.ChainRoot) {
        while(VertexChain_ptr !=NULL) { // Update Body Vertex by Skeleton Vertex chain
            struct PMD_VertexInformation *StartVertex_ptr = VertexChain_ptr->vStart;  // The Vertex chain in Master Chain
            struct PMD_VertexInformation *EndVertex_ptr   = VertexChain_ptr->vEnd;
            int nID = (StartVertex_ptr - Spirit->VertexInfo) * 3;

            do {
                float BoneWeight_0 = (float)StartVertex_ptr->BoneWeight *0.01f;
                float BoneWeight_1 = 1 -BoneWeight_0;
                struct SkeletonChainContext *AssociateBone_0 = MasterSkeletonCtx.ChainNodeSet +StartVertex_ptr->BoneID_0;
                struct SkeletonChainContext *AssociateBone_1 = MasterSkeletonCtx.ChainNodeSet +StartVertex_ptr->BoneID_1;

                //MulTransformMatrix(AssociateBone_0->TransformMatrix, BodyRederingCtx.Vertex +nID, BoneWeight_0, nVertex_0);
                //MulTransformMatrix(AssociateBone_1->TransformMatrix, BodyRederingCtx.Vertex +nID, BoneWeight_1, nVertex_1);
                MulTransformMatrix(AssociateBone_0->TransformMatrix, StartVertex_ptr->Vertex, BoneWeight_0, nVertex_0);
                MulTransformMatrix(AssociateBone_1->TransformMatrix, StartVertex_ptr->Vertex, BoneWeight_1, nVertex_1);
                nVertex_0[0] += nVertex_1[0];
                nVertex_0[1] += nVertex_1[1];
                nVertex_0[2] += nVertex_1[2];

                /* Update the GL vertex information */
                *(BodyRederingCtx.Vertex +nID)    = nVertex_0[0] ;
                *(BodyRederingCtx.Vertex +nID +1) = nVertex_0[1] ;
                *(BodyRederingCtx.Vertex +nID +2) = nVertex_0[2] ;
                /*StartVertex_ptr->Vertex[0]      = nVertex_0[0] ; //Update to Raw data (reserved code)
                StartVertex_ptr->Vertex[1]      = nVertex_0[1] ;
                StartVertex_ptr->Vertex[2]      = nVertex_0[2] ;*/
                StartVertex_ptr++;
                nID +=3;
            } while(StartVertex_ptr <= EndVertex_ptr);
            VertexChain_ptr = VertexChain_ptr->next;
        }
    }

    for(ci = 0; ci < SkeletonChain_ptr->ChildCount ; ci++) {
        struct SkeletonChainContext *tBone = *(SkeletonChain_ptr->Child +ci);
        update_BodySurfaceVertex(tBone);
    }
#endif
}
