...
generate_BoneMemory_Soruce_PMX(struct PolygonModeleXtendedContext *srcPMX)
{
    ...
    for(i = 0 ; i < srcPMX->BoneInfoCount ; i++) {
        ...
        BoneUnit_ptr->nameKey_DKDR = sutHash_DKDR(fetch_ptr->name_JP.text_UTF8, strlen(fetch_ptr->name_JP.text_UTF8));
        BoneUnit_ptr->nameKey_djb2 = sutHash_djb2(fetch_ptr->name_JP.text_UTF8, strlen(fetch_ptr->name_JP.text_UTF8));
        /*if(srcPMX->GlobalParamSet.TextEncoding ==0)
            nameLen = sutUTF16_to_UTF8(fetch_ptr->name_JP.text_UTF8, fetch_ptr->name_JP.Length, nameUTF8, defaultNameLen);
        else
            memcpy(nameUTF8, fetch_ptr->name_JP.text_UTF8, fetch_ptr->name_JP.Length);
...
static IplImage *load_Texture(char *fPath, char *fName)
{
    ...
    //IplImage *TextureImg = cvLoadImage("/home/Project/QtWorkspace/SEngine/Demo/Demo_AvatarMotion/Debug/PMX/jintsu/jintuu_arm.bmp", CV_LOAD_IMAGE_COLOR);
    if(!TextureImg) {
...
generate_TextureMemory_Soruce_PMX(struct PolygonModeleXtendedContext *srcPMX)
{
    ...
    for(i = 0 ; i < srcPMX->TextureInfoCount ; i++) {
        *Texture_ptr = (TextureContext *)malloc(sizeof(TextureContext));
        //IplImage *rImg = load_Texture(srcPMX->mPath, strlen(srcPMX->mPath), 8, TextureInfo->name_US.text_UTF8, TextureInfo->name_US.Length, srcPMX->GlobalParamSet.TextEncoding);
        ...
        (*Texture_ptr)->ByteLen = rImg->width * rImg->height * 3;
        (*Texture_ptr)->RGBA8888 = (unsigned char *)malloc(rImg->width * rImg->height *4);

        unsigned char *store_ptr = (*Texture_ptr)->RGBA8888;
        unsigned char *fetch_ptr = (unsigned char *)rImg->imageData;
...
generate_MaterialMemory_Soruce_PMX(struct PolygonModeleXtendedContext *srcPMX)
{
    ...
    for(i = 0 ; i < srcPMX->MaterialInfoCount ; i++) {
        MaterialUnit_ptr->JPnameKey_DKDR   = sutHash_DKDR(fetch_ptr->name_JP.text_UTF8, fetch_ptr->name_JP.Length);
        MaterialUnit_ptr->JPnameKey_djb2   = sutHash_djb2(fetch_ptr->name_JP.text_UTF8, fetch_ptr->name_JP.Length);
        MaterialUnit_ptr->Diffuse_RGBA[0]  = fetch_ptr->Diffuse_RGBA[0];
        MaterialUnit_ptr->Diffuse_RGBA[1]  = fetch_ptr->Diffuse_RGBA[1];
        MaterialUnit_ptr->Diffuse_RGBA[2]  = fetch_ptr->Diffuse_RGBA[2];
        MaterialUnit_ptr->Diffuse_RGBA[3]  = fetch_ptr->Diffuse_RGBA[3];
...
generate_VertexMemory_Soruce_PMX(struct PolygonModeleXtendedContext *srcPMX)
{
    ...
    for(i = 0 ; i < srcPMX->VertexInfoCount ; i++) {
        ...
        *StoreNoraml_ptr++ = fetch_ptr->Normal[1];
        *StoreNoraml_ptr++ = fetch_ptr->Normal[2];
        *StoreTexture_ptr++ = fetch_ptr->Texture[0];
        *StoreTexture_ptr++ = fetch_ptr->Texture[1];
        fetch_ptr++;
        VertexUnit_ptr++;
    }
    return 1;
