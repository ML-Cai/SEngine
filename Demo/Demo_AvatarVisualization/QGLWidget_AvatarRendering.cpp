                    ...
                    float *dstMatrix)
{
    ...
    eyeX = view_eye.length * angle_1_cos * cos(PI * view_eye.angle_2) ;
    eyeY = view_eye.length * sin(PI * view_eye.angle_1);
    eyeZ = view_eye.length * angle_1_cos * sin(PI * view_eye.angle_2);
    upY = angle_1_cos;
...
// ----------------------------------------------------------------------------------
void QGLWidget_AvatarRendering::Rendering_Skeleton()
{
    int i;
    GLSkeletonRenderingContext *SkeletonRenderingCtx = src_Avatar->get_SkeletonRenderingCtx();
    if(SkeletonRenderingCtx == NULL) return;
    ...
    for(i = 0; i < SkeletonRenderingCtx->JointCount ; i++) {
        glColor4fv(SkeletonRenderingCtx->JointColor +i*4);
        glVertex3fv(SkeletonRenderingCtx->JointVertex +i*3);
    }
    glEnd();
    glPopMatrix();
    ...
    glPushMatrix();
    glVertexPointer(3, GL_FLOAT, 0, SkeletonRenderingCtx->JointVertex);
    glColorPointer(4, GL_FLOAT, 0, SkeletonRenderingCtx->JointColor);
    glDrawElements(GL_LINES, SkeletonRenderingCtx->BoneCount, GL_UNSIGNED_SHORT, SkeletonRenderingCtx->BoneIndex );
    glPopMatrix();
...
void QGLWidget_AvatarRendering::paintGL()
{
    ...
    get_ModelViewMatrix(0.0f, 500.0f, 500.0f,
                        0.0f, 0.0f, 0.0f,
    ...
    if(src_Avatar != NULL) {
        src_Avatar->visualize(ProjectionMatrix, ModelViewMatrix);
        Rendering_Skeleton();
    }
}
// ----------------------------------------------------------------------------------
